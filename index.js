...
framework.hears(
  ...
  async (bot, trigger) => {
    ...
    bot.say("Snowflake Cortex bot is generating response. Please Wait ...");
    console.log("cortex command received");
    const jwt = new JWTGenerator().getToken();
    const agentUrl = process.env.AGENT_ENDPOINT;
    const semanticModels = process.env.REVENUE_SEMANTIC_MODEL;
    
    this.cortexChat = new CortexChat(jwt, agentUrl, semanticModels);
    const response = await this.cortexChat._retrieveResponse(trigger.text);
    response[0].forEach(async (contentItem) => {
      if (contentItem.type == 'text') {
        bot.say(contentItem.text);
      }
      else if (contentItem.type == 'suggestions') {
        contentItem.suggestions.forEach((suggItem) => {
          bot.say(suggItem);
        });
      }
      else if (contentItem.type=='sql') {
        //bot.say("markdown", contentItem.statement);
        const executor = new SnowflakeQueryExecutor();
        const df = await executor.runQuery(contentItem.statement);
        bot.say('markdown', df.toPSV(true));
      //bot.say('markdown', `You said: ${trigger.prompt}`);
       // await bot.say(df.toPSV(true));
    ...
    //bot.say("bot output goes here");
  },
  "**info**: (get detail from cortex bot)",
